...
void PhysObj::createBounds(box *face, vertex wpos)		//create cube bound around object
{
	// {min, max}
	/*
	float xm[2] = {face[0].point[0].x, face[0].point[0].x};
	float ym[2] = {face[0].point[0].y, face[0].point[0].y};
	float zm[2] = {face[0].point[0].z, face[0].point[0].z};
	*/
	...
	for(int x = 0; x < 2; x++)					//iterate through possible combinations of extrema
	{
		for(int y = 0; y < 2; y++)
		{
			...
			{
				bounds[ind] = vertex(wxm[x], wym[y], wzm[z], 1);
				//cout << ind << " : ( " << bounds[ind].x << " , " << bounds[ind].y << " , " << bounds[ind].z << " )" << endl;
				ind++;
			}
		}
	}
